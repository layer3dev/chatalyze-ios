...
class EventCell: ExtendedTableCell {
    ...
    func fillInfo(info:EventInfo?){
        ...
        if let startTime = DateParser.convertDateToDesiredFormat(date: info.start, ItsDateFormat: "yyyy-MM-dd'T'HH:mm:ss.SSSZ", requiredDateFormat: Locale.current.languageCode == "en" ? "EE, MMM dd h:mm" : Locale.current.languageCode == "zh" ? "yyyy 年 MM 月 d 日" : Locale.current.languageCode == "ko" ? "MM월 d일" : "dd/MM/yyyy"){
            
            self.eventtimeLbl?.text = startTime
            if let lastDate = DateParser.convertDateToDesiredFormat(date: info.end, ItsDateFormat: "yyyy-MM-dd'T'HH:mm:ss.SSSZ", requiredDateFormat:Locale.current.languageCode == "en" ? "h:mm a" : Locale.current.languageCode == "zh" ? "下午 h 點 mm 分" : Locale.current.languageCode == "th" ? "H.mm" : Locale.current.languageCode == "ko" ? (DateParser.convertDateToDesiredFormat(date: info.end, ItsDateFormat: "yyyy-MM-dd'T'HH:mm:ss.SSSZ", requiredDateFormat: "h:mm:ss a")?.contains("AM") ?? false) ? "오전 h시 mm분" : "오후 h시 mm분" : "H:mm"){
